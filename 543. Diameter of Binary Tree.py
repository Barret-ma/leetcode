...
class Solution(object):
    def diameterOfBinaryTree(self, root):
        ...
        if not root:
            return 0
        return self.travel(root.right, 0) + self.travel(root.left, 0)
    def travel(self, root, height):
        if not root:
            return height
        height += 1
        return max(self.travel(root.right, height), self.travel(root.left, height))
