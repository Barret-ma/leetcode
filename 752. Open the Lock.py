...
class Solution(object):
    def openLock(self, deadends, target):
        ...
        if '0000' in deadendsMap:
            return -1

        res = 0
        visited = set(['0000'])
        deadlocks = set(deadends)
        q = Queue(['0000'])
        q.put('0000')
        # print(q.qsize())
        while not q.empty():
            ...
            for i in range(q.qsize(), 0, -1):
                currentLocation = q.get()
                for charI in range(len(currentLocation)):
                    for dir in range(-1, 2):
                        if dir == 0: continue
                        clCopy = list(currentLocation)
                        # print((int(currentLocation[charI]) + 10 + dir) % 10)
                        clCopy[charI] = str((int(currentLocation[charI]) + 10 + dir) % 10)
                        clCopy = ''.join(clCopy)
                        if clCopy == target:
                            return res
                        if (clCopy not in visited and clCopy not in deadlocks): q.put(clCopy)
                        visited.add(clCopy)
        return -1
